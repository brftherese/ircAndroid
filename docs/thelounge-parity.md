...
2. **Server-side history + log storage** — Upstream persists scrollback via the `server/plugins/messageStorage/*` (SQLite/text) and exposes cleanup policies (`storageCleaner.ts`). We currently rely purely on in-memory Compose lists; closing the app drops history beyond mentions. Action: evaluate Room/SQLDelight log store or a lightweight replicator to a self-hosted Lounge instance.
