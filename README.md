...
- **Signed release APK:** Download `app-release.apk` and `app-release.apk.sha256` from the latest GitHub Release; both assets are generated by `.github/workflows/release.yml`.
- **Nightly builds:** Re-run the Release APK workflow via `workflow_dispatch` with a tag (e.g., `v1.2.0`) to regenerate artifacts after updating secrets.
